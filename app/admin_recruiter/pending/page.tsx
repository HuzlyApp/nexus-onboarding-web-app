...
export default function PendingCandidatesPage() {
  ...
  useEffect(() => {
    async function fetchPending() {
      setLoading(true);
      try {
        const res = await fetch("/api/workers?status=pending");
        ...
        console.error("Failed to fetch pending candidates:", e);
        setRows([]);
      } finally {
        setLoading(false);
      }
    ...
    <div className="flex h-screen bg-zinc-50 overflow-hidden">
      ...
      <div className="flex-1 flex flex-col overflow-hidden lg:pl-72">
        ...
        <div className="flex-1 p-8 overflow-auto">
          <div className="text-xs text-gray-600 mb-3">Admin - Pending Applicant Listings</div>
          ...
          <div className="bg-white border border-zinc-200 rounded-3xl overflow-hidden">
            <div className="px-6 py-4 border-b border-zinc-100 flex flex-wrap gap-3 items-center justify-between">
              <div className="flex flex-wrap items-center gap-3">
                <div className="text-sm text-gray-600">
                  Total: <span className="font-medium text-gray-600">{filtered.length}</span> applicants
                </div>
            ...
            {loading ? (
              <div className="text-center py-20 text-gray-600">Loading pending applicants...</div>
            ) : filtered.length === 0 ? (
              <div className="text-center py-20 text-gray-600">No pending applicants found.</div>
            ) : (
